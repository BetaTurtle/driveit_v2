...
// Google OAuth 2.0 credentials (for token exchange)
...
function checkEnvVariables() {
    ...
    if (!GOOGLE_OAUTH_CLIENT_ID) throw new Error("Missing environment variable: GOOGLE_OAUTH_CLIENT_ID");
    if (!GOOGLE_OAUTH_CLIENT_SECRET) throw new Error("Missing environment variable: GOOGLE_OAUTH_CLIENT_SECRET");
...
async function handler(req: Request): Promise<Response> {
    ...
    let googleAuth: { code: string; redirect_uri: string } | undefined;
    try {
        const body = await req.json();
        initDataString = body.initData as string;
        if (body.googleAuth && typeof body.googleAuth === 'object') {
            const { code, redirect_uri } = body.googleAuth as { code?: unknown; redirect_uri?: unknown };
    ...
    // --- Optionally exchange Google OAuth code ---
    let googleTokenPayload: Record<string, unknown> | undefined;
    if (googleAuth) {
        try {
            ...
            if (!tokenResp.ok) {
                console.warn("Google token exchange failed:", tokenJson);
                // Do not fail entire request; just omit google payload
            } else {
                googleTokenPayload = tokenJson as Record<string, unknown>;
                // Attempt to fetch userinfo to retrieve email
                try {
                    const accessToken = (tokenJson as Record<string, unknown>)["access_token"] as string | undefined;
                    if (accessToken) {
                        ...
                        if (userinfoResp.ok) {
                            ...
                            if (userinfo && typeof userinfo.email === 'string') {
                                (googleTokenPayload as Record<string, unknown>)["email"] = userinfo.email;
                                (googleTokenPayload as Record<string, unknown>)["email_verified"] = Boolean(userinfo.verified_email);
                            }
                        } else {
                            console.warn("Failed to fetch Google userinfo:", await userinfoResp.text());
                        }
                    }
                } catch (e) {
            ...
            console.warn("Google token exchange exception:", e?.message || e);
        }
    }
    ...
    // --- Success Response ---
    return new Response(JSON.stringify({ customToken: tokenResult.token, google: googleTokenPayload ?? null }), {
        status: 200,
        headers: { ...headers, 
